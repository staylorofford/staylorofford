...
def data_quality(site_code, location_code, channel_code, latitude, longitude, data_source, starttime, endtime,
                 splittime=None):
    ...
    if data_source == 'FDSN':
        ...
        # Find files matching the site, location, and channel codes

        found_files = []
        for root, dirs, files in walk(day_file_directory_root):
            for file in files:
                try:
                    st = read(root + '/' + file)
                    if (st[0].stats.station == site_code and
                            st[0].stats.location == location_code and
                            st[0].stats.channel == channel_code):
    ...
    elif data_source == 'local' or 'S3':

        # Here daily RMS values will be calculated for each site/location/channel code combination
...
starttime = '2020-01-01T00:00:00Z'  # starttime can either be an ISO8601 string, or an integer number of seconds to
                                    # query data for before the endtime.
endtime = '2020-12-31T23:59:59Z'  # endtime can be an ISO8601 string or 'now', if 'now' then endtime
...
# Set whether to get data from FDSN (short duration), local or S3. If S3 data will be downloaded for all full days
# between starttime and endtime inclusive.
data_source = 'S3'

# If you set data_source = 'local', provide path details to root folder containing all daily miniSEED files
day_file_directory_root = '/mnt/hgfs/VMSHARE/CRSZ testing/'
...
if data_source == 'FDSN':
    ...
    print('data_source needs to be set to either \'FDSN\', \'local\', or \'S3\' for the script to perform work.')
    exit()
    ...
    for m in range(len(site_metadata)):
        ...
        for n in range(len(channel_codes)):
            ...
            job = pp_pool.apply_async(data_quality, (site_code, location_code, channel_codes[n], latitude, longitude,
                                                     data_source, date,
                                                     date + datetime.timedelta(days=1, microseconds=-1)))
            jobs.append(job)
